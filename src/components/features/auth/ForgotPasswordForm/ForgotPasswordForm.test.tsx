...
describe('ForgotPasswordForm', () => {
  ...
  it('advances to step 2 on successful code send', async () => {
    render(<ForgotPasswordForm />);
    fireEvent.change(screen.getByLabelText('Email Address'), { target: { value: 'user@company.com' } });
  ...
  it('shows error message when code send fails', async () => {
    ...
    render(<ForgotPasswordForm />);
    fireEvent.change(screen.getByLabelText('Email Address'), { target: { value: 'bad@co.com' } });
    fireEvent.click(screen.getByRole('button', { name: 'Send Reset Link →' }));
  ...
  it('shows error message when reset code send fails', async () => {
    mockSendCode.mockResolvedValueOnce({ error: { message: 'Could not send code' } });
  ...
  it('shows error when code verification fails', async () => {
    mockVerifyCode.mockResolvedValueOnce({ error: { message: 'Invalid code' } });
  ...
  it('shows loading state while verifying code', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'New Password' })).toBeInTheDocument());
  });
  ...
  it('submits new password and shows success screen', async () => {
    ...
    await waitFor(() => {
      expect(mockSubmitPassword).toHaveBeenCalledWith({ password: 'newpass123' });
      expect(screen.getByText('Password Updated!')).toBeInTheDocument();
    });
  });

  it('shows error when password submit fails', async () => {
    mockSubmitPassword.mockResolvedValueOnce({ error: { message: 'Password too weak' } });
